...
import com.example.wy.tickto.calendar.MyCalendar;
...
public class MainActivity extends AppCompatActivity
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(mHour*100+mMinuts > writeDiaryTime_Hour*100+writeDiaryTime_Minute){
            //当前时间比设置时间晚，显示提醒
            long start = System.currentTimeMillis() + 2 * 1000;
...
//                        .setAction("Action", null).show();
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if(requestCode==DB_NOTADD){
            ...
            dateSt = data.getStringExtra("alarmTime");
            Log.d("MainActivity","添加事件的时间已传到主界面为："+dateSt);
...
//       }
           ...
           Toast.makeText(MainActivity.this, "  7Time，分享美好生活！", Toast.LENGTH_SHORT).show();
...
//        return super.onKeyDown(keyCode, event);
    ...
    public void getSettingTime(){
        List<ThingInfo> times = DataSupport.select("cal_date").find(ThingInfo.class);
        SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd hh:mm");
        Date now = new Date();
        for(ThingInfo time:times){
            String dateString = time.getCal_date();
            try{
                Date date = df.parse(dateString);
                Log.d("MainActivity","指定时间："+df.format(date));
                Log.d("MainActivity","指定时间毫秒级别为："+date.getTime());

            }catch (Exception ex){
                System.out.println(ex.getMessage());
            }
        }
        Log.d("MainActivity","系统时间："+df.format(now));
        Log.d("MainActivity","系统时间毫秒级别为："+now.getTime());
    ...
    public void getAlarm(View view) {
        ...
        Log.d("MainActivity", "闹钟已启动！");
        Toast.makeText(MainActivity.this, "未标记事件将在指定时间提醒！",Toast.LENGTH_SHORT).show();
        //得到事件的日期和id号
        List<ThingInfo> times = DataSupport.select("cal_date","id","isdone").find(ThingInfo.class);

        for(ThingInfo time:times) {

            boolean isAlarm = time.isDone();
            if(!isAlarm){
                //获取事件的时间和id 以及系统时间
                String dateString = time.getCal_date();
                int idInt = (int)time.getId();
                Log.d("MainActivity", "获取到可添加闹钟的事件"+ idInt +"的时间为：" + dateString);
                Date date = StrToDate(dateString);
                Date now = new Date();

                //调用闹钟，设定为毫秒时间后触发
                AlarmManager am = (AlarmManager) getSystemService(Context.ALARM_SERVICE);
                long timeDif = date.getTime() - now.getTime();
                if(timeDif>100) {
                    long triggerTime = System.currentTimeMillis() + timeDif; //在当前时间多少毫秒之后触发
                    Log.d("MainActivity", "时间点位于当前时间之后，且触发时长为" + timeDif + "毫秒后");
                    Intent intent = new Intent(this, AlarmClock.class);
                    //intent.setComponent(new ComponentName("com.example.wy.tickto.timingpush", "com.example.wy.tickto.timingpush.AlarmClock"));
                    PendingIntent op = PendingIntent.getBroadcast(this, idInt, intent, PendingIntent.FLAG_UPDATE_CURRENT);
                    am.set(AlarmManager.RTC, triggerTime, op);
                }else {
                    Log.d("MainActivity", "时间点位于当前时间之前，且触发时长为" + timeDif + "");
                }
            }
        }
    }
    ...
    public void cancelAlarm(View view) {
        ...
        Log.d("MainActivity", "闹钟已取消！");
        Toast.makeText(MainActivity.this, "提醒已取消！",Toast.LENGTH_SHORT).show();
        //得到事件的日期和id号
        List<ThingInfo> times = DataSupport.select("cal_date","id").find(ThingInfo.class);
        ...
        for(ThingInfo time:times) {
            ...
            String dateString = time.getCal_date();
            int idInt = (int)time.getId();
            Log.d("MainActivity", "获取到事件"+ idInt +"的时间为：" + dateString);
            ...
            PendingIntent op = PendingIntent.getBroadcast(this, idInt, intent, PendingIntent.FLAG_UPDATE_CURRENT);
            am.cancel(op);
            Log.d("MainActivity", "闹钟"+ idInt +"已取消");
        }
    }
